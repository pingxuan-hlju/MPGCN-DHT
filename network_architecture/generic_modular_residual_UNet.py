...
from .ITT import LGI_GT
from nnunet.network_architecture.Med3D.test import ImageProcessor
...
class ResidualUNetEncoder(nn.Module):
    def __init__(self, input_channels, base_num_features, num_blocks_per_stage, feat_map_mul_on_downscale,
        ...
        edge_num_types = 8  # 构建边的维度
        
        self.lgt = LGI_GT(num_layers, t_layers, num_heads, depth, height, width, node_num, in_features, dropout, edge_num_types)
        ...
        height_2 = 28
        width_2 = 28
        node_num_2 = 12544  # 结点个数
        in_features_2 = 320  # 特征维度
        dropout_2 = 0.1  # dropout率，越大丢弃越多
        edge_num_types_2 = 8  # 构建边的维度
        self.lgt2 = LGI_GT(num_layers_2, t_layers_2, num_heads_2, depth_2, height_2, width_2, node_num_2 , in_features_2, dropout_2, edge_num_types_2)
    ...
    def forward(self, x, return_skips=None):
        ...
        """
        skips = []
 #     初始 torch.Size([2, 1, 32, 224, 224])
#每一层 torch.Size([2, 32, 32, 224, 224])
#每一层 torch.Size([2, 32, 32, 224, 224])
#每一层 torch.Size([2, 64, 32, 112, 112])
#每一层 torch.Size([2, 128, 32, 56, 56])
#每一层 torch.Size([2, 256, 16, 28, 28])
#每一层 torch.Size([2, 320, 8, 14, 14])
        # ==============================================================
        target_size = (128, 128, 128)
        processor = ImageProcessor(model_type='vit_b_ori', device='cuda', checkpoint=None, img_size=128)
        # print("x.shape",x.shape)
        stretched_tensor, scale_factor = processor.stretch_tensor(x, target_size)  # stretched_tensor torch.Size([2, 32, 128, 128, 128])
        # print("stretched_tensor",stretched_tensor.shape)
        image_embedding = processor.predict(stretched_tensor) 
        # print("image_embedding",image_embedding.shape)
        unstretched_tensor = processor.unstretch_tensor(image_embedding, scale_factor)  #  # torch.Size([2, 384, 2, 12, 12]) 224/16
        
        
        # unstretched_tensor = torch.nn.functional.interpolate(unstretched_tensor, size=(8,14,14),
        #                                                      mode='trilinear', align_corners=False) #  # torch.Size([2, 384, 2, 14, 14]) 
        bs,channel,depth,height,width = unstretched_tensor.size()
        unstretched_tensor = unstretched_tensor.reshape(bs,96,8,height,width)  # torch.Size([2, 96, 8, 12, 12])  384/4=96 2*4=8
        # padding = (0, 0, 0, 0, 1,1)  # torch.Size([2, 320, 3, 5, 7]) -> torch.Size([2, 320, 4, 5, 7])  (left, right, top, bottom, front, back)
        # unstretched_tensor = F.pad(unstretched_tensor, padding)  # torch.Size([2, 96, 8, 14, 12]) -> torch.Size([2, 96, 10, 12, 12]) 
        
        unstretched_tensor =  self.covn_change(unstretched_tensor)  # torch.Size([2, 320, 10, 12, 12]) 
        unstretched_tensor = self.bn1(unstretched_tensor)
        # =====================================================
        # print("初始",x.shape)
...
#                 x = self.lgt2(x1).reshape(x_shape.shape)
            if w==14:
                ...
                x_shape = x
                x1 = x.reshape(x.shape[0],x.shape[1],-1)  # 1*320*196
                x = self.lgt(x1).reshape(x_shape.shape)
...
def find_3d_configuration():
    ...
    optimizer = SGD(unet.parameters(), lr=0.01, momentum=0.95) #原本0.1
